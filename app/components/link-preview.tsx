...
// This component handles the hover state and calculates position to escape 'overflow: hidden' containers
export const LinkPreview = ({ 
  ...
  const handleMouseEnter = () => {
    if (triggerRef.current) {
      ...
      setCoords({
        ...
        top: rect.top - 10, // 10px gap above the link
      });
      setIsOpen(true);
  ...
  return (
    <>
      ...
      {/* Portal-like behavior: We render fixed to escape overflow containers */}
        ...
        >
          ...
          <div className="animate-in fade-in zoom-in-95 duration-200">
              ...
              <div className="relative w-full aspect-video bg-zinc-100 rounded-lg overflow-hidden border border-zinc-100">
                {isLoading ? (
                  ...
                  <div className="w-full h-full bg-zinc-50 flex flex-col items-center justify-center gap-2">
                     ...
                     <span className="text-zinc-400 text-[10px] font-mono">{href.replace(/^https?:\/\//, '')}</span>
                  </div>
                )}
              </div>
            ...
            <div className="w-full flex justify-center -mt-px">
               <div className="w-3 h-3 bg-white border-b border-r border-zinc-200 transform rotate-45 translate-y-[-50%]"></div>
            </div>
          </div>
        </div>
      )}
